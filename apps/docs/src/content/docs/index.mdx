...
hero:
  ...
  actions:
    - text: Get Started
      link: /prose/getting-started/
      icon: right-arrow
      variant: primary
    - text: View on GitHub
      link: https://github.com/celom/prose
      icon: external
      variant: minimal
---
...
<CardGrid stagger>
  ...
  <Card title="Retries & Backoff" icon="random">
    Per-step retry policies with exponential backoff, delay caps, and
  ...
  <Card title="Durable Execution" icon="seti:db">
    Opt-in skip-ahead checkpointing — survive process crashes without re-running
    completed steps.
  </Card>
</CardGrid>
