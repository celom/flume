...
<Aside type="caution">
  `MemoryDurabilityStore` is a reference implementation. It survives nothing —
  the moment the process exits, every checkpoint is gone. Use it in tests; reach
  for a persistent adapter in production.
</Aside>
...
<Aside type="caution">
  `MemoryDurabilityStore` survives nothing. The idempotency contract above is
  what keeps a real adapter safe across crashes.
</Aside>
