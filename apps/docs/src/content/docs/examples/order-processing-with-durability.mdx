...
<Aside type="caution">
  `MemoryDurabilityStore` is fine for tests and dev. It does not survive a
  process exit — for real crash recovery, implement a persistent
  [`DurabilityStore`](/prose/api/durability-store/) backed by your database.
</Aside>
