...
export function registerResources(server: McpServer) {
  ...
  server.registerResource(
    ...
    async (uri) => ({
      contents: [
        { uri: uri.href, text: QUICK_REFERENCE, mimeType: 'text/markdown' },
      ],
    })
  );
  ...
  server.registerResource(
    'api-create-flow',
    'prose://api/create-flow',
    {
      description:
        'API reference for createFlow() — the entry point for building flows',
      mimeType: 'text/markdown',
    },
    async (uri) => ({
      contents: [
  ...
  server.registerResource(
    ...
    async (uri) => ({
      contents: [
        { uri: uri.href, text: TYPES_REFERENCE, mimeType: 'text/markdown' },
      ],
    })
  );
  ...
  server.registerResource(
    ...
    'prose://api/execution-options',
    {
      description:
        'API reference for FlowExecutionOptions passed to flow.execute()',
      mimeType: 'text/markdown',
    },
    async (uri) => ({
      contents: [
        {
          uri: uri.href,
          text: EXECUTION_OPTIONS_REFERENCE,
          mimeType: 'text/markdown',
        },
      ],
    })
  ...
  server.registerResource(
    ...
    async (uri) => ({
      contents: [
        { uri: uri.href, text: OBSERVERS_REFERENCE, mimeType: 'text/markdown' },
      ],
    })
  );

  server.registerResource(
    'api-durability-store',
    'prose://api/durability-store',
    {
      description:
